...
for raster_path in glob.glob(r"F:/Odisha_data/OPTRAM/data2/mask_final/*.tif"):

    filename = os.path.basename(raster_path)
    #year = filename.split("_")[-1][:-4]
    project =  filename.split("_")[-1][:-4]
    print(project)
    csv_path = f"F:/Odisha_data/OPTRAM/data2/mask_final/{filename[:-4]}.csv"
    ...
    for no_bands in range(data.shape[0]):
        ...
        output_path_parent = f'F:/Odisha_data/OPTRAM/data2/new_data/{project}/2022_23/{project}_Irrigation_coverageRabbi'

        if not os.path.exists(output_path_parent):
            os.makedirs(output_path_parent)

        output_path = output_path_parent + "/" + project + "RabliIndexCoverage_"+ date_list[no_bands].replace("-","") + '.tif'
        #sprint(output_path)
