...
farms_csv = pd.read_file()


farms_shp = farms_shp.reindex(sorted(farms_shp.columns), axis=1)
farms_shp = farms_shp.fillna(value=0)

filter_col = [col for col in farms_shp if col.startswith('VI')]

date_wise = farms_shp[filter_col]

for index, row in farms_shp.iterrows():
    df = row[filter_col]
    print("SG filter ongoing")
    final_18_19 = main_process(df)
    print("SG filter done")

    farms_shp.loc[index, filter_col] = final_18_19[0]

    #break

farms_shp.to_file(r'E:/shubham/shp/TN_6vill_vill4_selected_farms_ndvi_smooth.shp')
