...
def main_process(df):
    ...
    #smooth = savitzky_golay_filtering(original_list)
    #print('smooth',len(smooth))
    Expanded_timeseries = Timeseries_Expand(original_list, MVC_pol, SG_gap_arr)
    #print('Expanded_timeseries',len(Expanded_timeseries))
    Expanded_timeseries_gapfil = fill_gap(Expanded_timeseries)
    final_list.append(Expanded_timeseries_gapfil)
...
farms_csv = pd.read_csv('F:/TN_6vill/zonal_statistics/shp/TIRUPPACHCHANUR_field_boundary_UUID_farmUUID_ndvi_med.csv')


farms_csv = farms_csv.reindex(sorted(farms_csv.columns), axis=1)
farms_csv = farms_csv.fillna(value=0)

filter_col = [col for col in farms_csv if col.startswith('VI')]

date_wise = farms_csv[filter_col]

for index, row in farms_csv.iterrows():
    df = row[filter_col]
    print("SG filter ongoing")
    final_18_19 = main_process(df)
    print("SG filter done")

    farms_csv.loc[index, filter_col] = final_18_19[0]

    #break

farms_csv.to_csv('F:/TN_6vill/zonal_statistics/shp/TIRUPPACHCHANUR_field_boundary_UUID_farmUUID_ndvi_med_smooth.csv')
